...
\begin{note}{Selection Sort}
	...
	\end{tabular}%
	\end{mdframed}
	\incode{selection-sort} is an inefficient sorting algorithm
 ...
 for (i from 0 to n-2)
	...
	\tags{Sorting, Comparison}
\end{note}

% Buuble sort = lots of swaps 
% Insertion sort = insert current element into right position in sorted array
...
\begin{note}{Disjoint-Set Data Structure}
	...
	\tags{Data Structure, Set, Disjoint-Set, Union-Find}
\end{note}

\begin{note}{Disjoint-Set Structure Find}
	The \incode{find} function of a \hyperref[note:Disjoint-Set Data Structure]{\it disjoint-set} data structure
	determines the representative of the set for a given element \incode{x}.
	\begin{largecode}
 find(x):
 	if (x.parent != x) x.parent = find(x.parent)
	return x.parent
	\end{largecode}
	\begin{remark} This \incode{find} uses {\it path compression} to flatten the tree structure.
...
\begin{note}{Disjoint-Set Structure Union}
	The \incode{union} function of a \hyperref[note:Disjoint-Set Data Structure]{\it disjoint-set} data structure merges two sets
...
\begin{note}{Kruskal's Algorithm}
 ...
 for (u,v) in E ordered by weight:
	...
	\tags{Minimum Spanning Tree, Union-Find, Greedy}
\end{note}
...
\begin{note}{Hierholzer Algorithm}
 ...
 TODO!
	 \end{largecode}
	 \vspace{-5pt}
	 \tags{Graphs, Traversal, Single-Source, Euler Circuit}
\end{note}
...
\begin{note}{Edit Distance}
	Given two strings \incode{x}, \incode{y}, determine their {\it edit distance}, i.e.
	the minimum number of character operations $\in$ \{\incode{insert}, \incode{replace}, \incode{delete}\}
...
\begin{note}{Edit Distance Algorithm}
	...
	through \incode{dp[i,j]} defined as the edit distance between the substrings \incode{x[..i-1]} and \incode{y[..j-1]}.
	$$
		d_{i,j} = \begin{dcases}
	...
	\end{remark}
	\vspace{-5pt}
	\tags{Solution, Dynamic Programming, Edit Distance}
\end{note}

\begin{note}{Distinct Subsequences}
	Given two strings \incode{s} and \incode{t}, determine
	the number of distinct subsequences of \incode{t} in \incode{s}.
...
\begin{note}{Distinct Subsequences DP}
	The number of \hyperref[note:Distinct Subsequences]{\it distinct subsequences} of \incode{t} in \incode{s}
	is determined through dynamic programming using \incode{dp[i,j]} as the number of distinct subsequences
	of \incode{t[..j-1]} in \incode{s[..i-1]}.
	$$
		d_{i,j} = \begin{dcases}
			d_{i-1, j} & \text{ if } s_{i-1} \neq s_{j-1}\\
			d_{i-1, j} + d_{i-1, j-1} & \text{ otherwise}
		\end{dcases}
	$$
	\vspace{-5pt}
	\begin{remark}
		Initialize with \incode{dp[i,j] = (j == 0) ? 1 : 0}.
	\end{remark}
	\vspace{-5pt}
	\tags{Solution, Dynamic Programming, Distinct Subsequences}
\end{note}
...
\begin{note}{Meeting Rooms Algorithm}
	...
	we {\it encode} each timestamp $t \in \{s_i, f_i\}$ to a pair \incode{(t, \{-1, 1\})}.
...
\begin{note}{LL to BST Solution}
 ...
 toBST(root, NULL)  // for solution
	\end{largecode}
	\vspace{-5pt}
	\tags{Solution, Linked-List, Binary Search Tree, Tortoise and Hare}
\end{note}
...
\begin{note}{Sorted Matrix Search}
	Given a two-dimensional $m \times n$ matrix \incode{M} such that each row and column is sorted,
	determine whether \incode{M} contains the target \incode{t}.
...
\begin{note}{Linear Sorted Matrix Search}
	To search an element in a \hyperref[note:Sorted Matrix Search]{\it sorted matrix} \incode{M}
	in linear $O(m + n)$ runtime, begin from the bottom left corner and move each axis in
	only one direction, according to \incode{M[x][y]} and \incode{target}.
 ...
 while (x >= 0 and y <= n)
	...
	\end{largecode}
	\begin{remark}
		Think about why we can't miss \incode{target}.
	\end{remark}
	\vspace{-5pt}
	\tags{Solution, Matrix, Search}
\end{note}
...
\begin{note}{Reservoir Sampling}
	...
	memory, we apply the {\it reservoir sampling} or \incode{R} algorithm: replace a random element from the
	reservoir with every new element $s_i$ with probability $p = k/i$.
%	\begin{enumerate}
% \item  pick new element $s_i$ with probability $p = k/i$
% \item if $s_i$ is selected, remove a random element from the reservoir
%	\end{enumerate}
	\begin{largecode}
 for i from k+1 to n
 ...
 $$
	\vspace{-10pt}
	\tags{Solution, Random Sampling, Stream}
\end{note}

\begin{note}{Reverse Interleave Linked List}
	Given a singly linked list \incode{L}, reorder it 
...
\begin{note}{Reverse Interleave Linked List Solution}
	A linked list \incode{L} can be \hyperref[note:Reverse Interleave Linked List]{\it reversely interleaved}
 ...
 return merge(root, end)
	\end{largecode}
	\begin{remark} Linear in-place \incode{reverse} and \incode{merge} left as exercise.
		A similar approach can determine if \incode{L} is \emph{was} a palindrome.
	\end{remark}
	\vspace{-5pt}
	\tags{Solution, Linked List, Merge, Reverse, Interleave, Tortoise and the Hare}
\end{note}

\begin{note}{Binary Tree Levels}
	Given the \incode{root} of a binary tree, return a list of its levels or layers.
...
\begin{note}{Binary Tree Level Traversal}
 ...
 while queue:
	...
	\vspace{-5pt}
	\tags{Solution, Breadth-First Search, Queue, Levels, Binary Tree}
\end{note}

\begin{note}{Binary Tree Right View}
	Given the \incode{root} of a binary tree, return
...
\begin{note}{Symmetric Binary Tree}
	Given the \incode{root} to a binary tree, determine whether the tree is symmetric along
...
\begin{note}{Recursive Binary Tree Symmetry}
 ...
 bool symmetric(left, right)
	...
	\end{largecode}
	\vspace{-5pt}
	\tags{Solution, Binary Tree, Symmetric, Recursion}
\end{note}

\begin{note}{Lowest Common Ancestor}
	Given the \incode{root} to a {\it binary tree} and two nodes \incode{p} and \incode{q},
...
\begin{note}{Recursive Lowest Common Ancestor}
 ...
 lca(n, p, q):
	...
	\vspace{-5pt}
	\tags{Solution, Binary Tree, Lowest Common Ancestor, Recursion}
\end{note}

\begin{note}{Validate Postorder}
	Given an array of nodes \incode{A}, determine whether it corresponds
...
\begin{note}{Validate Postorder Algorithm}
	To determine whether \incode{A} is a \hyperref[note:Validate Postorder]{\it validate postorder} traversal,
	we use a stack \incode{s} which progressively replaces leaves by \incode{\#}.
	\begin{largecode}
for n in nodes:
	if (n == "#")
 ...
 return s == ["#"]
	...
	\vspace{-5pt}
	\tags{Solution, Binary Tree, Postorder, Traversal}
\end{note}

\begin{note}{Sum Root to Leaf Numbers}
	Given the \incode{root} to a binary tree of nodes \incode{0-9}, every path $p$
...
\begin{note}{Sum Roof to Leaf Numbers Solution}
 ...
 sum(node, acc):
	...
	\end{largecode}
	\begin{remark} \incode{acc} could also just be a reference, which we would need to restore it to
		its original value at the end of each call. \end{remark}
	\vspace{-5pt}
	\tags{Solution, Binary Tree, Depth-First Search, Recursion}
\end{note}

\begin{note}{Minimum Height Tree}
	Given an undirected acyclic graph with \incode{n} nodes and a list of edges \incode{(i, j)}
...
\begin{note}{Minimum Height Tree Algorithm}
 ...
 return graph.nodes
	\end{largecode}
	\vspace{-5pt}
	\tags{Solution, Minimum Height Tree}
\end{note}

% Course Schedule maybe, although that's so obvious

% Fake Head for Linked Lists == Sentinel basically.
% Labels <= when you have element -> set(elements) and all those elements also -> set(...)

\begin{note}{Two Stock Transactions}
	\favorite
	Given an array where \incode{p[i]} is the price of some stock on day \incode{i},
...
\begin{note}{Linear Two Stock Optimization}
	...
	by building \incode{left[i]} and \incode{right[i]} representing the maximum profit with a single transaction
	respectively before and after day \incode{i}.
 ...
 return max(left[i] + right[i] for i from 0 to n-1)
	...
	\end{remark}
	\vspace{-5pt}
	\tags{Solution, Dynamic Programming, Stock}
\end{note}

\begin{note}{First Missing Positive}
	Given an array \incode{A}, determine the lowest strictly positive integer not present in the array
...
\begin{note}{Linear Missing Positive}
	Determine the \hyperref[note:First Missing Positive]{\it first missing positive} of \incode{A} by
	placing each positive element \incode{x} into position \incode{x-1}.
 ...
 return n+1
	\end{largecode}
	\begin{remark} Linear time complexity because every element \incode{A[i]} is placed at its correct position exactly once. \end{remark}
	\vspace{-5pt}
	\tags{Solution, First Missing Positive, Swapping, Sort}
\end{note}

\begin{note}{Sliding Window Maximum}
	Given an array of integers \incode{A} and the size \incode{k} of a sliding window,
...
\begin{note}{Sliding Window Maximum}
	...
	use a \emph{double-ended} queue \incode{Q} whose front will alway contain the current maximum.
	For every new element \incode{A[i]} we remove all elements from the back which are smaller.
 ...
 for (i from 0 to n-1)
	...
	\end{largecode}
	\vspace{-5pt}
	\tags{Solution, Sliding Window, Queue, Double-Ended Queue}
\end{note}

\begin{note}{LRU Cache}
	Implement a \emph{least recently used} cache with capacity $k$
	supporting \incode{read} and \incode{set} operations.
		\begin{largecode}
 input: k = 3
...
\begin{note}{LRU Cache Implementation}
	...
	a doubly-linked list as a priority queue \incode{Q} in addition to a map \incode{M}
	from \incode{keys} to \incode{(value, pointer)} pairs giving random access to the entry in \incode{Q}.
	\begin{largecode}
 void set(key, value):
...
\vspace{-5pt}
 ...
 int read(key):
	...
	\end{largecode}
	\vspace{-5pt}
	\tags{Solution, LRU Cache}
\end{note}

\begin{note}{Maximum Sum Sub-Matrix}
	Given a 2-dimensional matrix \incode{M} of integers,
...
\begin{note}{Kadane's Matrix Algorithm}
	...
	by using a 2D prefix-sum matrix \incode{s[i,j]} and applying
 ...
 for (top from 0 to n-1)
	...
	\begin{remark} The code above highlights the reduction to Kadane, but
		should use \incode{s} in pratice instead of recomputing \incode{A}.
	\end{remark}
	\vspace{-5pt}
	\tags{Solution, Kadane Algorithm, Matrix, Maximum Sum, Prefix Sum}
\end{note}
...
\begin{note}{Linear Famous Person}
	...
	in linear time by process of elimination. Use two pointers \incode{i, j} traveling towards each other
	from opposite sides of \incode{V}.
	\begin{largecode}
 while (i < j)
 	if (edge[i][j]) i++ else j--
	\end{largecode}
	\begin{remark} If no adjacency matrix is given, iterate over the edges \incode{E}  and shrink a set \incode{S} of candidates.\end{remark}
	\vspace{-5pt}
	\tags{Solution, Famous Person}
\end{note}

\begin{note}{User Group Relationships}
	Given a map \incode{input} mapping groups to a list
	of users \emph{and} groups, expand the map to \incode{group:users}.
	\begin{largecode}
...
\begin{note}{User Group Relationships DFS}
	...
	we can produce an expanded \incode{group:users} mapping in linear time by using \hyperref[note:Depth-First Search]{\it depth-first search}.
	\begin{largecode}
 visit(node, path):
 	if node.isUser():
...
\begin{note}{Type Conversion Queries}
	Given a list of type conversions \incode{(type1, type2, rate)},
	implement a fast and space efficient \incode{convert} function.
	\begin{largecode}
...
\begin{note}{Efficient Type Conversions}
 ...
 for (t1, t2, rate) in conversions:
	...
	\end{largecode}
	\vspace{-5pt}
	\begin{remark} The difficulty lies in the implementation of \incode{union}. \end{remark}
	\vspace{-5pt}
	\tags{Solution, Union-Find, Disjoint Set}
\end{note}

%\begin{note}{Weighted Sampling}
%	\tags{Problem}
%\end{note}
...
\begin{answer}{Interview Etiquette}
	...
	\end{itemize}
	\tags{Interviews, Behavior, Best Practices}
\end{answer}
